                ...
                <tab-group header="密码管理">
                    ...
                    <tab header="备用密码管理">
                        <table class="table ready-pass">
                            <tr v-for="item in bycode[page_count]" track-by="$index" @click="changeCodeBtnShow($index)" :class="{ 'tr-click':bycode_btn_isshow[page_count][$index].showY || bycode_btn_isshow[page_count][$index].showN }">
                                <td :class="{ 'no-set':!item}" >{{page_count=='first'?$index+1:$index+9}}</td>
                                <td :class="{ 'no-set':!item}" >{{item?item.name:'无'}}</td>
                                <td>
                    ...
                    <div class="modal-item">
                        ...
                        <div class="fl">
                            <select type="text" v-model="add_TC.opentime" class="form-control">
                                ...
                                <option value="-1" selected="selected">永久有效</option>
                            </select>
                        </div>
                    </div>
        ...
        lockOperation: function(operation,type){
            ...
            if(operation == "SetPermaLockPWD"){//设置/重制备用密码
                ...
                if(type == "add"){//设置租客密码
                    ...
                    if(this.add_TC.opentime!=1 && this.add_TC.opentime!=-1){
                        showMsg(this.$store, '请选择失效时间！', 'warning');
                        return;
                    }
                    ...
                    if(this.modify_TC.opentime!=1 && this.modify_TC.opentime!=-1){
                        showMsg(this.$store, '请选择失效时间！', 'warning');
                        return;
                    }
                    let date_tmp = time.replace(/-/g, "/");
